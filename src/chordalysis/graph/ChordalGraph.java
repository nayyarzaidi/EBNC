...
public class ChordalGraph extends SimpleGraph<Integer, DefaultEdge> implements UndirectedGraph<Integer, DefaultEdge> {
	...
	/**
	 * To use with extreme caution, and only if the structure is not modified
	 * afterwards
	 * 
	 * @param vertex1
	 * @param vertex2
	...
	public DefaultEdge addSecuredEdge(Integer a, Integer b) {
		// if (a == 3 && b == 7) {
		// return addSecuredEdge(a, b, true);
		// } else {
		// return addSecuredEdge(a, b, false);
	...
	public DefaultEdge addSecuredEdgeOld(Integer a, Integer b, boolean verbose) {
		...
		if (verbose) System.out.println("\tSab=" + Sab + "\t" + SabSet);

		// new clique
		BitSet Cab = new BitSet(edgeSet().size());
		Cab.or(Sab);
		Cab.set(a);
		Cab.set(b);
		...
		ConnectivityInspector<Integer, DefaultEdge> inspectorGp = new ConnectivityInspector<Integer, DefaultEdge>(this);
		...
		for (CliqueGraphEdge cEdge : cgEdges) {
			BitSet c1 = cEdge.c1;
			BitSet c2 = cEdge.c2;
			...
			if (S12.equals(Sab)) {
				...
				if (verbose) System.out.println("\tis there a path between " + vFromC1mS12 + " and " + vFromC2mS12 + " if remove" + S12 + "?" + inspectorGpp.pathExists(vFromC1mS12, vFromC2mS12));
		...
		SabUnionb.set(b);
		for (CliqueGraphEdge CpCb : cg.edgesOf(Cb)) {
			BitSet Cp = CpCb.c1;
			if (Cp.equals(Cb)) {
				Cp = CpCb.c2;
			}
			...
			if (verbose) System.out.println("\tCp=" + Cp + " is a neighbour of Cb=" + Cb);
			BitSet CpInterCb = CpCb.separator;
			if (verbose) System.out.println("\tCp inter Cb=" + CpInterCb);
			if (CpInterCb.isEmpty()) {
				continue;
			}

			if (containsAll(SabUnionb, CpInterCb)) {
				if (!cg.containsEdge(Cp, Cab) && !cg.containsEdge(Cab, Cp)) {
					if (verbose) System.out.println("\tadd edge (C',Cab)b=(" + Cp + "," + Cab + ")");
					CliqueGraphEdge addedEdge = cg.addEdge(Cp, Cab);
					if (addedEdge.separator.isEmpty()) {
						System.err.println("shouldn't have added this edge");
						System.exit(0);
		...
		for (BitSet Cp : cg.vertexSet()) {
			BitSet Sp = (BitSet) Cp.clone();
			Sp.and(Cab);
			if (!Sp.isEmpty() && Sp.equals(SabUnionb)) {
				// check if Sab union a separates b from C'-Cab
				BitSet CpMinusCab = (BitSet) Cp.clone();
				CpMinusCab.andNot(Sp);
				if (CpMinusCab.isEmpty()) {
				...
				// copy graph for test
				SimpleGraph<Integer, DefaultEdge> graphCopy = (SimpleGraph<Integer, DefaultEdge>) super.clone();
				for (int v = SabUnionb.nextSetBit(0); v >= 0; v = SabUnionb.nextSetBit(v + 1)) {
					graphCopy.removeVertex(v);
				}
				...
				ConnectivityInspector<Integer, DefaultEdge> inspector = new ConnectivityInspector<Integer, DefaultEdge>(graphCopy);
				// first component
				Set<Integer> componentOfA = inspector.connectedSetOf(a);
				// second component
				Integer oneVertex = CpMinusCab.nextSetBit(0);
				Set<Integer> componentOfCpMinusCab = inspector.connectedSetOf(oneVertex);

				if (!componentOfA.equals(componentOfCpMinusCab)) {
					if (!cg.containsEdge(Cp, Cab) && !cg.containsEdge(Cab, Cp)) {
						if (verbose) System.out.println("\tadd edge (Cab,C') = (" + Cab + "," + Cp + ")");
						CliqueGraphEdge addedEdge = cg.addEdge(Cp, Cab);
						if (addedEdge.separator.isEmpty()) {
		...
		if (verbose) System.out.println("cliques: " + cg.vertexSet());
	 ...
	 */
	@Deprecated
	private void updateEligibleEdges() {

		for (int i = 0; i < eligibleEdges.length; i++) {
			for (int j = i + 1; j < eligibleEdges.length; j++) {
				eligibleEdges[i][j] = null;
				eligibleEdges[j][i] = null;
			}
		}

		ConnectivityInspector<Integer, DefaultEdge> insp = new ConnectivityInspector<Integer, DefaultEdge>(this);
		for (int i = 0; i < eligibleEdges.length; i++) {
			BitSet connectedToI = new BitSet();
			for (Integer v : insp.connectedSetOf(i)) {
				connectedToI.set(v);
			}
			for (int j = i + 1; j < eligibleEdges.length; j++) {
				if (!connectedToI.get(j)) {
					// different connected component

					// find cliques for each vertex
					BitSet Ci = null;
					BitSet Cj = null;
		...
		for (CliqueGraphEdge edge : cg.edgeSet()) {
			...
			for (int va = c1.nextSetBit(0); va >= 0; va = c1.nextSetBit(va + 1)) {
				if (edge.separator.get(va)) continue;
				for (int vb = c2.nextSetBit(0); vb >= 0; vb = c2.nextSetBit(vb + 1)) {
					if (edge.separator.get(vb)) continue;
					eligibleEdges[va][vb] = new CliqueGraphEdge(c1, c2);
					eligibleEdges[vb][va] = eligibleEdges[va][vb];
				}
			}
		}
	...
	public SimpleGraph<BitSet, CliqueGraphEdge> getJunctionTree() {
		...
		for (CliqueGraphEdge edge : cg.edgeSet()) {
			BitSet source = cg.getEdgeSource(edge);
			BitSet target = cg.getEdgeTarget(edge);
			DefaultWeightedEdge newEdge = wcg.addEdge(source, target);
			wcg.setEdgeWeight(newEdge, -edge.separator.size());// negate for
																// finding
																// maximum
																// spanning tree
		}
	...
	private void disableEdge(Integer a, Integer b, MyPriorityQueue pq, boolean verbose) {
		if (eligibleEdges[a][b] != null || eligibleEdges[b][a] != null) {
	 ...
	 */
	public ArrayList<Integer> getEliminationOrdering() {
		if (peo != null) {
			return peo;
		} else {
	 ...
	 * @return a possible elimination ordering on the graph.
	 */
	public ArrayList<Integer> getEliminationOrdering(int[]preferredOrder) {
		peo = new ArrayList<Integer>();
		...
		// the
		// labels of all the nodes (with priority to initial order)
		for (int i = 0; i < preferredOrder.length; i++) {
			int vertex = preferredOrder[i];
			labels.put(vertex, new Integer(0));
			order.put(vertex, i);
		}
		while (!labels.isEmpty()) {
			...
			for (Entry<Integer, Integer> entry : labels.entrySet()) {
				...
				int currentPosition = order.get(currentVertex);
				
				if (currentValue > maxNumber || (currentValue == maxNumber && currentPosition > bestPosition)) {
					xi = currentVertex;
					maxNumber = currentValue;
					bestPosition = currentPosition;
				}
			}
//			System.out.println("adding "+xi+" at the start of peo ("+maxNumber+","+bestPosition+")");
			peo.add(0, xi);
	...
	public DirectedAcyclicGraph<Integer, DefaultEdge> getBayesianNetwork() throws CycleFoundException {
		return getBayesianNetwork(null);
	 ...
	 * @throws CycleFoundException
	 */
	public DirectedAcyclicGraph<Integer, DefaultEdge> getBayesianNetwork(int []preferredOrder) throws CycleFoundException {

		List<Integer> peo;
		if(preferredOrder==null){
			peo= getEliminationOrdering();
		}else{
			peo = getEliminationOrdering(preferredOrder);
		}
	...
	public static void main(String[] args) throws CycleFoundException {
		...
		// graph.addVertex(i);
		// }
		// System.out.println(graph.cg);
		//
		// graph.addEdge(0,1);
		// graph.addEdge(1,2);
		// graph.addEdge(2,3);
		// graph.printAvailableEdges();
		//
		//
		// for (int i = 0; i < 4; i++) {
		// for (int j = i + 1; j < 4; j++) {
		...
		System.out.println(graph.cg);
		System.out.println("peo="+graph.getEliminationOrdering());
		System.out.println("peo="+graph.getEliminationOrdering(new int[]{6,3,0,2,4,5,1}));
		System.out.println(graph.getCliquesBFS());
		System.out.println(graph.getSeparatorsBFS());
		System.out.println(graph.getJunctionTree());
