...
public class ParallelObjectiveFunctionMSE_d extends ObjectiveFunctionMSE_d {
	...
	public ParallelObjectiveFunctionMSE_d(wdBayes algorithm) {
		super(algorithm);
	...
	public FunctionValues getValues(double params[]) {
		...
		for (int th = 0; th < nThreads; th++) {
			 ...
			 * Calling thread
			 */
			Callable<Double> thread = new CallableMSE_d(algorithm.getM_Instances(), start, stop, algorithm.getNc(), nodes[th], tmpProbs[th], gs[th], algorithm.getdParameters_(), algorithm.getM_Order(), algorithm);
