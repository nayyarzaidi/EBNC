...
public class ParallelObjectiveFunctionCLL_e extends ObjectiveFunctionCLL_e {
	...
	public ParallelObjectiveFunctionCLL_e(wdBayes algorithm) {
		super(algorithm);
	...
	public FunctionValues getValues(double params[]) {
		...
		for (int th = 0; th < nThreads; th++) {
			 ...
			 * Calling thread
			 */
			Callable<Double> thread = new CallableCLL_e(algorithm.getM_Instances(), start, stop, algorithm.getNc(), nodes[th], tmpProbs[th], gs[th], algorithm.getdParameters_(), algorithm.getM_Order(), algorithm);
